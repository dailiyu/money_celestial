    ...
    <view class="content">
      ...
      <view class="head_box flex_between" style="align-items: flex-start">
        ...
        <upload
          ...
          :imgWidth="800"
          :imgHeight="800"
          :imgUrls="avatarImages"
          @uploadSuccessfulPaths="acceptSuccessProfileImgPath"
        ></upload>
      </view>
      <view class="head_box">
        <view class="flex_between" style="margin-bottom: 54rpx">
          ...
          <view class="tips_text"> 第一张请上传门面照片 </view>
        </view>
        <upload
          amount="6"
          :imgWidth="900"
          :imgHeight="600"
          :imgUrls="bannerImages"
          @uploadSuccessfulPaths="acceptSuccessBannerImgPath"
        ></upload>
      </view>
      <view class="head_box">
        <view class="flex_between" style="margin-bottom: 54rpx">
          <view class="h_title"> 店铺营业执照照片 </view>
        </view>
        <upload
          amount="6"
          :showUpload="false"
          :imgUrls="authfileImages"
        ></upload>
      </view>
      <view class="shop_info" @click="showTips">
        <view class="info_item flex_between">
          <view class="s_title"> 统一社会信用代码 </view>
          <!-- <input
				disabled
      ...
      <view class="shop_info">
        <view class="info_item flex_between">
          <view class="s_title"> 赠送百分比 </view>
          <input
            v-model="proportion_gift"
            @blur="validateInput"
            class="uni-input"
      ...
      <view class="head_box">
        <view class="shop_intro">
          <view class="h_title" style="margin-bottom: 34rpx"> 店铺介绍 </view>
          <textarea
            v-model="shopIntro"
            placeholder="请输入商家介绍"
...
const bannerImages = shopInfo.images
  .filter((image) => image.image_type === "banner")
  .map((image) => image.image_url);
const detailImages = shopInfo.images
  .filter((image) => image.image_type === "other")
  .map((image) => image.image_url);
const avatarImages = shopInfo.images
  .filter((image) => image.image_type === "avatar")
  .map((image) => image.image_url);
const authfileImages = shopInfo.images
  .filter((image) => image.image_type === "authfile")
  .map((image) => image.image_url);
...
const onChange = (e) => {
  ...
  const province = cityData.value.find(
    (item) => item.value === selected[0].value
  );
  const city = province?.children?.find(
    (item) => item.value === selected[1].value
  );
...
const associatedBannerImg = async () => {
  ...
  for (let i = 0; i < successBannerImgPaths.value.length; i++) {
    bannerListUrl.value.push({
      image_url: successBannerImgPaths.value[i],
      image_type: "banner",
    });
  }
...
const associatedAuthfileImg = async () => {
  ...
  for (let i = 0; i < successAuthfileImgPaths.value.length; i++) {
    authfileListUrl.value.push({
      image_url: successAuthfileImgPaths.value[i],
      image_type: "authfile",
    });
  }
...
const associatedDetailImg = async () => {
  for (let i = 0; i < successDetailImgPaths.value.length; i++) {
    detailListUrl.value.push({
      image_url: successDetailImgPaths.value[i],
      image_type: "other",
    });
  }
...
const associatedProfileImg = async () => {
  ...
  userProfileUrls.value.push({
    image_url: successProfileImgPaths.value[0],
    image_type: "avatar",
  });
...
const saveStoreInfo = async () => {
  ...
  try {
    ...
    const res = await changeShopInfo(phoneNumber, {
      merchant: phoneNumber,
      categories: [businessRange.value],
      name: shopName.value,
      description: shopIntro.value,
      avatar: profileUrl.value,
      address: address.value,
      license_no: business_license.value,
      consume2coin_bit: proportion_gift.value,
    ...
    const params = [
      ...bannerListUrl.value,
      ...detailListUrl.value,
      ...userProfileUrls.value,
      ...authfileListUrl.value,
    ];
