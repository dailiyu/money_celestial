<template>
	<view class="points-record-page flex-col">
		...
		<image
			...
			src="https://static.maxcang.com/appstatic/my/green_point/green_point_record_bg.png"
			mode="widthFix"
			style="top: -88rpx;"
		/>
		<!-- 主要内容区域 -->
		<view class="main-content-area flex-col">
			...
			<view class="points-display-area flex-col">
				...
				<view class="points-header flex-row justify-between">
					<text class="current-points-label">当前绿色积分</text>
					<image
						class="info-help-icon"
						src="https://static.maxcang.com/appstatic/my/green_point/info_icon.png"
						mode="aspectFit"
					/>
				</view>
				<text class="total-points-number">{{pointData.green_points || '0'}}</text>
			</view>
			<!-- 标签页和记录列表容器 -->
...
const refreshing = ref(false) // 新增：控制下拉刷新状态
...
const getTransactions = async (page = 1, append = false) => {
	try {
		...
		const params = {
			page: page,
			page_size: 10,
			transaction_method: 'green_points', // 绿积分
...
const mapTransactionData = (apiData) => {
	return apiData.map(item => {
		...
		return {
			lanhutext0: item.transaction_remark || '积分交易',
			lanhutext1: statusText,
			lanhutext2: displayAccount, // 脱敏后的账户信息
			lanhutext3: `${isIncrease ? '+' : '-'}${amount.toFixed(2)}`,
			lanhufontColor3: isIncrease ? 'rgba(42,194,164,1)' : 'rgba(205,78,69,1)',
			lanhutext4: formatDateTime(item.created_at),
			lanhutext5: `价值：$${(amount * 0.01).toFixed(2)}`, // 假设1积分=0.27美元
