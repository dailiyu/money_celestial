		...
		<view class="search_filter_container">
			...
			<view class="filter_section">
				<view class="filter_item">
					...
					<uni-data-select v-model="category" :localdata="range" placeholder="全部类目" :clear="false" @change="changeRange"></uni-data-select>
				</view>
				<view class="filter_item">
					...
					<view class="sort_arrows" @click="toggleTimeSort">
						<image src="https://static.maxcang.com/appstatic/arrow-inactive.png" mode="widthFix" class="arrow_icon arrow_up" :class="{'active': timeSort === 'asc'}" style="transform: rotate(180deg);"></image>
						<image src="https://static.maxcang.com/appstatic/arrow-inactive.png" mode="widthFix" class="arrow_icon arrow_down" :class="{'active': timeSort === 'desc'}"></image>
					</view>
				</view>
				<view class="filter_item">
					...
					<view class="sort_arrows" @click="toggleRewardSort">
						<image src="https://static.maxcang.com/appstatic/arrow-inactive.png" mode="widthFix" class="arrow_icon arrow_up" :class="{'active': rewardSort === 'asc'}" style="transform: rotate(180deg);"></image>
						<image src="https://static.maxcang.com/appstatic/arrow-inactive.png" mode="widthFix" class="arrow_icon arrow_down" :class="{'active': rewardSort === 'desc'}"></image>
					</view>
				</view>
			</view>
		</view>
						...
						<uni-swipe-action-item v-for="(item, index) in filteredShopList" :key="item.shop_detail.merchant">
							<template #right>
								<view class="swipe_right" @click="del(item)">
									<image src="https://static.maxcang.com/appstatic/my/delete.png" mode="widthFix" class="del_pic"></image>
									<view>删除</view>
								</view>
							</template>
								...
								<view class="store-info">
									<view class="store-header">
										<text class="store-name">{{ item.shop_detail.name }}</text>
																			<view class="store-badge" v-if="item.shop_detail.pay_cert_material_state === 1">
...
const getShopList = async()=>{
	...
	try {
		const res = await browserShopsList(categoryId.value, ordering, page.value)
		if (page.value === 1) {
...
const del = async(item)=>{
	try {
		...
		uni.hideLoading()
		uni.showToast({ title: e.data.detail, icon: 'none' })
	}
}
