  ...
  const _sfc_main$R = {
    ...
    setup(__props) {
      ...
      const login = async () => {
        ...
        userStore2.loginAction(moblie.value, password.value).then((res) => {
          uni.hideLoading();
          uni.showToast({
            title: "登录成功",
            icon: "success",
  ...
  const _sfc_main$Q = {
    ...
    setup(__props) {
      ...
      vue.onMounted(async () => {
        const accessToken = uni.getStorageSync("accessToken");
        if (accessToken) {
          await publicStore.fetchAllDataAction(), await userStore2.fetchAllDataAction();
        }
        uni.getLocation({
          geocode: true,
          success(res) {
      ...
      const toMerchant = () => {
        if (userStore2.storeInfo && Object.keys(userStore2.storeInfo).length > 0) {
          formatAppLog("log", "at pages/index/index.vue:200", userStore2.storeInfo);
          uni.navigateTo({
      ...
      const toAgent = () => {
        if (userStore2.userInfo.is_province_agent || userStore2.userInfo.is_city_agent) {
          uni.navigateTo({
      ...
      const toRecommend = () => {
        if (userStore2.userInfo.is_referral_officer) {
          uni.navigateTo({
      ...
      return (_ctx, _cache) => {
        ...
        return vue.openBlock(), vue.createElementBlock("view", { class: "page" }, [
          ...
          vue.createElementVNode("view", { class: "content" }, [
            ...
            vue.unref(userStore2).storeInfo && Object.keys(vue.unref(userStore2).storeInfo).length > 0 ? (vue.openBlock(), vue.createElementBlock("view", {
              key: 1,
              class: "merchant_box"
            }, [
...
详细信息：${s4}`), r3(i3);
    ...
    setup(__props) {
      ...
      const merchantSettleIn = async () => {
        ...
        try {
          ...
          await userStore2.fetchAllDataAction();
          uni.hideLoading();
          uni.showToast({
            title: "入驻成功",
            duration: 600,
        ...
        return vue.openBlock(), vue.createElementBlock("view", null, [
          ...
          vue.createElementVNode("view", { class: "content" }, [
            vue.createElementVNode("view", { class: "merchant_info flex" }, [
              vue.createElementVNode("image", {
                src: vue.unref(userStore2).storeInfo.avatar,
                mode: "aspectFill",
                class: "head"
              }, null, 8, ["src"]),
              vue.createElementVNode("view", { class: "" }, [
                vue.createElementVNode("view", { class: "name flex" }, [
                  vue.createElementVNode(
                    "view",
                    { class: "" },
                    vue.toDisplayString(vue.unref(userStore2).storeInfo.name),
                    1
                    /* TEXT */
                  ),
              ...
              vue.createElementVNode("view", { class: "content" }, [
                vue.createElementVNode("view", {
                  ...
                  vue.createElementVNode("view", { class: "avtar" }, [
                    vue.createElementVNode("image", {
                      class: "img",
                      src: vue.unref(userStore2).userInfo.icon,
                      mode: ""
                    }, null, 8, ["src"])
                  ]),
                  vue.createElementVNode(
                    "view",
                    { class: "name" },
                    vue.toDisplayString(vue.unref(userStore2).userInfo.username || "点击登录"),
                    1
                    /* TEXT */
                  )
                ]),
      ...
      const toRegister = async () => {
        ...
        if (password.value == password2.value) {
          postRegister(mobile.value, password.value).then((res) => {
            ...
            userStore2.loginAction(mobile.value, password.value);
            setTimeout(() => {
              uni.navigateTo({
  ...
  const _sfc_main$c = {
    ...
    setup(__props) {
      ...
      account.value = userStore2.userInfo.username;
      vue.onMounted(async () => {
      });
      const isChecked = vue.ref(false);
      const changeCheck = () => {
